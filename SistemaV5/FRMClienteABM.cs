...
namespace SistemaV5
{
    public partial class FRMClienteABM : Form
    {
        ...
        private bool ClienteTieneReservasActivas(int idClienteAEliminar)
        {
            // Instancia del repositorio de reservas
            IReservasRepositorio _reservaRepositorio = new ArchivoReservasRepositorio();

            List<CLSReserva> todasLasReservas = _reservaRepositorio.GetAllReservas();

            foreach (CLSReserva reserva in todasLasReservas)
            {
                // Aca se verifican dos cosas:
                // 1. Que la reserva pertenezca al cliente (comparando IDs)
                // 2. Que la reserva esté "activa" 

                if (reserva.IdCliente == idClienteAEliminar && reserva.Estado == 1) // <-- Verificación
                {
                    return true; // Encontró al menos una reserva activa
                }
                // Otra posible verificación si tienes fechas:
                if (reserva.IdCliente == idClienteAEliminar && reserva.FechaReserva >= DateTime.Today)
                {
                    return true;
                }
            }
            return false; // No se encontraron reservas activas para este cliente
        }
